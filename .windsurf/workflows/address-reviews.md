...
find the current repo name with repo_name=$(gh pr view --json headRepository --jq .headRepository.name)

Get the current unresolved review threads with `gh pr-review review view --unresolved -R $repo_owner/$repo_name $pr_number`
...
For each open thread - Resolve them with `gh pr-review threads resolve --repo $repo_owner/$repo_name --pr $pr_number --thread-id <thread id>`. Beware you need to close each thread individually
